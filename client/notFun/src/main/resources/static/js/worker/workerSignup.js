...
memberEmail.addEventListener("input", (e) => {
  ...
  if (inputEmail.trim().length === 0) {
    workerEmailMessage.innerText = "메일을 받을 수 있는 이메일을 입력해주세요.";

    // 메시지에 색상을 추가하는 클래스 모두 제거
    workerEmailMessage.classList.remove("confirm", "error");
  ...
  if (!regExp.test(inputEmail)) {
    workerEmailMessage.innerText = "알맞은 이메일 형식으로 작성해주세요.";
    workerEmailMessage.classList.add("error"); // 글자를 빨간색으로 변경
    workerEmailMessage.classList.remove("confirm"); // 초록색 제거
  ...
  fetch("/worker/checkEmail?memberEmail=" + inputEmail)
    ...
    .then((count) => {
      ...
      if (count == 1) {
        // 증복이면
        workerEmailMessage.innerText = "이미 사용중인 이메일 입니다.";
...
const workerIdMessage = document.querySelector("#workerIdMessage");
const checkWorkerIdBtn = document.querySelector("#checkWorkerIdBtn");
...
checkWorkerIdBtn.addEventListener("click", () => {
  ...
  if (!regExp.test(inputId)) {
    workerIdMessage.innerText = "알맞은 아이디 형식으로 작성해주세요.";
    workerIdMessage.classList.add("error"); // 글자를 빨간색으로 변경
    workerIdMessage.classList.remove("confirm"); // 초록색 제거
    checkObj.workerId = false; // 유효하지 않은 이메일임을 기록
    return;
  }
  ...
  fetch("/worker/checkWorkerId?workerId=" + inputId)
    ...
    .then((count) => {
      ...
      if (count == 1) {
        // 증복이면
        workerIdMessage.innerText = "이미 사용중인 아이디 입니다.";
        workerIdMessage.classList.add("error");
        workerIdMessage.classList.remove("confirm");
        checkObj.workerId = false; // 중복은 유효하지 않은 상태이다.
        return;
      }
      // 중복 X인 경우
      workerIdMessage.innerText = "사용 가능한 아이디 입니다";
      workerIdMessage.classList.add("confirm");
      workerIdMessage.classList.remove("error");
      checkObj.workerId = true; // 유효한 이메일
    })
    .catch((error) => {
      // fetch 수행 중 예외 발생 시 처리
      console.log(error); // 발생한 예외 출력
...
workerNickname.addEventListener("input", (e) => {
