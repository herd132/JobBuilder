...
const fetchRequest = async () => {
  ...
  try {
    ...
    if (boardTitles.length > 0) {
      ...
      if (currentPage === 1) {
        // 첫 번째 페이지 로딩 시 데이터가 없으면 "작성한 글이 없습니다." 메시지 표시
        showNoDataMessage();
      }
...
function makeBoardList(boardTitles) {
  ...
  boardTitles.forEach((board) => {
    ...
    // 글번호
    const writeNo = document.createElement("td");
    writeNo.textContent = board.boardNo; // boardNo

    // 제목
    const title = document.createElement("td");
    title.textContent = board.boardTitle; // boardTitle
    ...
    date.textContent = board.boardWriteDate.split(" ")[0]; // boardWriteDate (날짜 부분만)

    // 조회수
    const viewCount = document.createElement("td");
    viewCount.textContent = board.boardReadCount; // boardReadCount

    // 행에 각 셀 추가
    row.appendChild(writeNo);
    row.appendChild(title);
    row.appendChild(date);
    row.appendChild(viewCount);
