...
public class EmailConfig {
	...
	// cf) @Autowired 는 Bean 으로 등록한 객체만 사용 가능!!
	...
	@Bean		// 이런 설정들이 적용된 JavaMailSender를 Bean 으로 등록하여 Spring Container 가 관리
	public JavaMailSender javaMailSender() {
		
		/* Spring 에서 JavaMailSender 를 구성하는 Bean 정의 
		 * 
		 * JavaMailSender : 이메일을 보내는데 사용되는 인터페이스로,
		 * 		JavaMailSenderImpl 클래스를 통해 구현됨
		 * 
		 * SMTP 서버(맨아래쪽 설명 참고)를 사용하여 이메일을 보내기 위한 구성을 제공
		 * 
		 * */
		...
		return mailSender;
		// 위처럼 각종 설정이 적용된 JavaMailSender를 Bean 으로 등록하여
	 ...
	 * 3. Google SMTP 서버가 이메일 주소에 맞는 최종 수신자의
	 *	  이메일 서버(예: Gmail, Yahoo, 회사 이메일 서버)로 이메일을 다시 보냄.
	 * 4. 수신자가 이메일을 받음
	 * 
	 * 
	 * (참고)
